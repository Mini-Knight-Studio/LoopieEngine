...
namespace Loopie
{
	...
	void RectTransform::SetAnchorMin(vec2& anchorMin)
	{
		m_anchorMin = Clamp01(anchorMin);
		MarkWorldDirty();
	}

	void RectTransform::SetAnchorMax(vec2& anchorMax)
	{
	...
	vec2 RectTransform::GetParentRectMin() const
	{
		const auto parent = GetOwner()->GetParent().lock();
		if (!parent)
			return vec2(0.0f);
	...
	vec2 RectTransform::GetComputedSize() const
	{
	...
	vec2 RectTransform::GetAnchorReferencePoint(const vec2& parentSize) const
	{
	...
	vec2 RectTransform::ComputePivotTranslation(const vec2& size) const
	{
		return -(m_pivot * size);
	}
	...
	vec3 RectTransform::GetLocalBoundsMin() const
	{
	...
	vec3 RectTransform::GetLocalBoundsMax() const
	{
		const vec2 size = GetComputedSize();
		const vec2 min = ComputePivotTranslation(size);
	...
	void RectTransform::RefreshMatrices() const
	{
		if (!IsDirty()) return;
	...
	void RectTransform::Clone(const std::shared_ptr<Entity> entity, const Component& other)
	{
